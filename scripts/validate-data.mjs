...
if (data.theories.length < 367) throw new Error(`Expected exhaustive-breadth baseline >= 367 theories; found only ${data.theories.length}`);

if (new Set(data.theories.map(t=>t.category)).size < 13) throw new Error("Expected at least 13 top-level categories");
...
if (data.sources.length < 358) throw new Error(`Expected breadth-baseline bibliography >= 358 sources; found only ${data.sources.length}`);
