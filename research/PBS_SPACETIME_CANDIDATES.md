# PBS Space Time theory/model candidate audit

_Last reviewed: 2026-09-22_

The Search PBS Space Time index contains more than a thousand suggested phrases spanning physics, astronomy, mathematics and general vocabulary. It is explicitly **not treated as an exhaustive theory list**.

This file extracts theory/model/hypothesis-like phrases that are useful for Quantum Index discovery.

## Already represented in Quantum Index

| PBS indexed phrase / transcript concept | Quantum Index disposition |
|---|---|
| Bohmian mechanics | existing: `bohmian` |
| Bohr model | existing: `bohr-model` |
| Born rule | existing: `born-rule` |
| collapse theory / collapse model / collapse interpretation | existing collapse family + GRW/CSL/QMUPL variants |
| Diósi–Penrose model | existing: `diosi-penrose` / gravitational-collapse variants |
| many worlds | existing: `everett` |
| string theory | existing: `string-theory` |
| Standard Model | existing: `standard-model` |
| FLRW universe | existing: `flrw-cosmology` |
| Lambda-CDM model / concordance model | existing: `lambda-cdm` |
| Lemaître–Tolman universe | existing: `ltb-cosmology` |
| Kerr solution | existing: `kerr-spacetime` |
| Schwarzschild solution | existing: `schwarzschild-spacetime` |
| cosmic censorship / cosmic censorship hypothesis | existing: `cosmic-censorship` |
...
| quantum mechanics | represented through the core formulation/foundations graph |
| Hawking radiation | existing: `hawking-radiation` |
| black-hole information paradox | represented through Hawking/island/fuzzball/holography relations; candidate for a dedicated problem node rather than a theory node |

## Candidate: scientific first-class entity review
...
| Drake equation | **formula only** | astrophysical/astrobiology estimator rather than physical theory |

## Transcript-driven new astrophysics coverage

This audit directly motivated or corroborated the current astrophysics expansion:

- general relativity
- Schwarzschild and Kerr spacetimes
- FLRW and LTB cosmologies
- ΛCDM
- MOND and TeVeS
...
## Next PBS search sweep

Search the transcript index systematically against:

1. every current theory canonical name;
2. every alias;
3. every theory tag;
4. unresolved candidate terms above;
5. astrophysics terms including accretion, neutron stars, white dwarfs, MHD, synchrotron, cosmic rays, halo profiles and structure formation.

Promote a transcript hit to `pbs-spacetime.js` only after the episode/title/date and the mention itself are verified.
